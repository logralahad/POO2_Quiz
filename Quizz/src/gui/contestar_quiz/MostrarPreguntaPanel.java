 ...
 * @author logra
 */
public class MostrarPreguntaPanel extends JSplitPane{
    ...
    public MostrarPreguntaPanel(String pregunta, ArrayList<Inciso> incisos){
    ...
    public String obtenerPregunta(){
        return pnlPregunta.getText();
    }
    
    public ArrayList<String> obtenerSelecciones(){
        ArrayList<String> seleccionadas = new ArrayList<>();
        for (int i = 0; i < pnlRespuestas.getComponentCount(); i++) {
            MostrarIncisoPanel opcion = (MostrarIncisoPanel)pnlRespuestas.getComponent(i);
            if(opcion.getCorrecto()){
                seleccionadas.add(opcion.getTexto());
            }
        }
